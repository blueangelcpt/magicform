		...
		if (!empty($object)) {
			...
			if (!empty($object->validate)) {
				foreach ($object->validate as $validateField => $validateProperties) {
					$messages[$validateField] = $this->_getValidationMessage($validateProperties);
					if ($this->_isRequiredField($validateProperties)) {
						$validates[] = $validateField;
					}
				}
			}
			$defaults = array('fields' => array(), 'key' => 'id', 'validates' => array(), 'messages' => array());
...
/**
 * Returns the validation message for a field
 */
	function _getValidationMessage($validateProperties) {
		$message = false;
		if (is_array($validateProperties)) {
			...
			foreach ($validateProperties as $field => $validateProp) {
				$message = isset($validateProp['message']) ? $validateProp['message'] : false;
				if ($message) {
					break;
				}
			}
